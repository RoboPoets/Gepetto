...
class RetargetingPanel(ToolPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (
            not context.scene.rsl_retargeting_armature_source
            or not context.scene.rsl_retargeting_armature_target
        ):
        ...
        if not context.scene.rsl_retargeting_bone_list:
            ...
            row.scale_y = 1.2
            row.operator(
                retargeting.BuildBoneList.bl_idname,
                icon_value=Icons.CALIBRATE.get_icon(),
            )
            self.draw_import_export(layout)
            return
        ...
        row = layout.row(align=True)
        row.operator(
            retargeting.AddBoneListItem.bl_idname, text="Add Custom Entry", icon="ADD"
        )
        ...
        row.scale_y = 1.4
        row.operator(
            retargeting.RetargetAnimation.bl_idname,
            icon_value=Icons.CALIBRATE.get_icon(),
        )
...
class RSL_UL_BoneList(UIList):
    def draw_item(
        ...
        if item.is_custom:
            layout.prop_search(
                item, "bone_name_source", armature_source.pose, "bones", text=""
            )
        else:
        ...
        if armature_target:
            layout.prop_search(
                item, "bone_name_target", armature_target.pose, "bones", text=""
            )
