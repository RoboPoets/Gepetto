...
from ..properties import BoneListItem
...
class BuildBoneList(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if (
            not armature_source.animation_data
            or not armature_source.animation_data.action
        ):
            self.report(
                {"ERROR"},
...
class AddBoneListItem(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        context.scene.rsl_retargeting_bone_list_index = (
            len(context.scene.rsl_retargeting_bone_list) - 1
        )
        return {"FINISHED"}
...
class RetargetAnimation(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for bone in context.object.data.edit_bones:
            ...
            bone_transforms[bone.name] = (
                ...
                utils.mat3_to_vec_roll(
                    armature_source.matrix_world.inverted().to_3x3()
                    @ bone.matrix.to_3x3()
                ),
        ...
        for item in self.retarget_bone_list:
            ...
            bone_new = armature_source.data.edit_bones.new(
                item.bone_name_target + RETARGET_ID
            )
            bone_new.head, bone_new.tail, bone_new.roll = bone_transforms[
        ...
        # Change action name
        armature_target.animation_data.action.name = (
